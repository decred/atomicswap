...
func getFeePerKb(c *rpc.Client) (useFee, relayFee monautil.Amount, err error) {
	...
	relayFee, err = monautil.NewAmount(info.RelayFee)
	if err != nil {
		return 0, 0, err
	}
	...
	if info.PaytxFee != 0 {
		...
		useFee, err = monautil.NewAmount(maxFee)
	...
	if err == nil && estimateResp != -1 {
		useFee, err = monautil.NewAmount(estimateResp)
		if relayFee > useFee {
			useFee = relayFee
		}
		return useFee, relayFee, err
	...
	useFee, err = monautil.NewAmount(info.RelayFee)
	return useFee, relayFee, err
}
