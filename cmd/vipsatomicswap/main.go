...
// Copyright (c) 2019 The VIPSTARCOIN developers
...
import (
	...
	"github.com/vipstar-dev/vipswallet/wallet/txrules"
...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of VIPSTARCOIN Core wallet RPC server")
...
// VIPSTARCOIN transactions for these swaps.  A second tool should be used for the
// transaction on the other chain.  Any chain can be used so long as it supports
// OP_SHA256 and OP_CHECKLOCKTIMEVERIFY.
//
// Example scenerios using vipstarcoin as the second chain:
//
// Scenerio 1:
//   cp1 initiates (dcr)
//   cp2 participates with cp1 H(S) (vips)
//   cp1 redeems vips revealing S
...
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems vips with S

func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: vipsatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
type initiateCmd struct {
	cp2Addr *vipsutil.AddressPubKeyHash
	amount  vipsutil.Amount
}

type participateCmd struct {
	cp1Addr    *vipsutil.AddressPubKeyHash
	amount     vipsutil.Amount
	secretHash []byte
}
...
func run() (err error, showUsage bool) {
	...
	switch args[0] {
	case "initiate":
		cp2Addr, err := vipsutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp2AddrP2PKH, ok := cp2Addr.(*vipsutil.AddressPubKeyHash)
		if !ok {
		...
		amount, err := vipsutil.NewAmount(amountF64)
		if err != nil {
			return err, true
		}
	...
	case "participate":
		cp1Addr, err := vipsutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp1AddrP2PKH, ok := cp1Addr.(*vipsutil.AddressPubKeyHash)
		if !ok {
...
// pubkey for a transaction input signature.  Due to limitations of the VIPSTARCOIN
// Core RPC API, this requires dumping a private key and signing in the client,
// rather than letting the wallet sign.
func createSig(tx *wire.MsgTx, idx int, pkScript []byte, addr vipsutil.Address,
...
// vipsd/rpcclient package.
func fundRawTransaction(c *rpc.Client, tx *wire.MsgTx, feePerKb vipsutil.Amount) (fundedTx *wire.MsgTx, fee vipsutil.Amount, err error) {
	...
	feeAmount, err := vipsutil.NewAmount(resp.Fee)
	if err != nil {
		return nil, 0, err
	}
...
func getFeePerKb(c *rpc.Client) (useFee, relayFee vipsutil.Amount, err error) {
	...
	relayFee, err = vipsutil.NewAmount(netInfoResp.RelayFee)
	if err != nil {
		return 0, 0, err
	}
	payTxFee, err := vipsutil.NewAmount(walletInfoResp.PayTxFee)
	if err != nil {
		return 0, 0, err
	}
	...
	if err == nil && estimateResp.FeeRate > 0 {
		useFee, err = vipsutil.NewAmount(estimateResp.FeeRate)
		if relayFee > useFee {
			useFee = relayFee
		}
...
// account parameter which was removed in VIPSTARCOIN Core 0.15.
func getRawChangeAddress(c *rpc.Client) (vipsutil.Address, error) {
	...
	addr, err := vipsutil.DecodeAddress(addrStr, chainParams)
	if err != nil {
		return nil, err
	}
	...
	if _, ok := addr.(*vipsutil.AddressPubKeyHash); !ok {
...
type contractArgs struct {
	them       *vipsutil.AddressPubKeyHash
	amount     vipsutil.Amount
	locktime   int64
	secretHash []byte
}
...
type builtContract struct {
	contract       []byte
	contractP2SH   vipsutil.Address
	contractTxHash *chainhash.Hash
	contractTx     *wire.MsgTx
	contractFee    vipsutil.Amount
	refundTx       *wire.MsgTx
	refundFee      vipsutil.Amount
}
...
func buildContract(c *rpc.Client, args *contractArgs) (*builtContract, error) {
	...
	contractP2SH, err := vipsutil.NewAddressScriptHash(contract, chainParams)
	if err != nil {
		return nil, err
	}
...
func buildRefund(c *rpc.Client, contract []byte, contractTx *wire.MsgTx, feePerKb, minFeePerKb vipsutil.Amount) (
	refundTx *wire.MsgTx, refundFee vipsutil.Amount, err error) {
	...
	refundAddr, err := vipsutil.NewAddressPubKeyHash(pushes.RefundHash160[:], chainParams)
	if err != nil {
		return nil, 0, err
	}
	...
	if txrules.IsDustOutput(refundTx.TxOut[0], minFeePerKb) {
		return nil, 0, fmt.Errorf("refund output value of %v is dust", vipsutil.Amount(refundTx.TxOut[0].Value))
	}
...
func calcFeePerKb(absoluteFee vipsutil.Amount, serializeSize int) float64 {
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Contract fee: %v (%0.8f VIPS/kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.8f VIPS/kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	recipientAddr, err := vipsutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	contractHash := vipsutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if sc == txscript.ScriptHashTy &&
			bytes.Equal(addrs[0].(*vipsutil.AddressScriptHash).Hash160()[:], contractHash) {
			contractOut = i
			break
		}
	}
	...
	if txrules.IsDustOutput(redeemTx.TxOut[0], minFeePerKb) {
		return fmt.Errorf("redeem output value of %v is dust", vipsutil.Amount(redeemTx.TxOut[0].Value))
	}
	...
	fmt.Printf("Redeem fee: %v (%0.8f VIPS/kB)\n\n", fee, redeemFeePerKb)
...
func (cmd *auditContractCmd) runOfflineCommand() error {
	contractHash160 := vipsutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if bytes.Equal(addrs[0].(*vipsutil.AddressScriptHash).Hash160()[:], contractHash160) {
			contractOut = i
			break
		}
	}
	...
	contractAddr, err := vipsutil.NewAddressScriptHash(cmd.contract, chainParams)
	if err != nil {
		return err
	}
	recipientAddr, err := vipsutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
	...
	refundAddr, err := vipsutil.NewAddressPubKeyHash(pushes.RefundHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	...
	fmt.Printf("Contract value:          %v\n", vipsutil.Amount(cmd.contractTx.TxOut[contractOut].Value))
