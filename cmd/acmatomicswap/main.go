...
// Copyright (c) 2019 The Actinium developers
...
import (
	...
	rpc "github.com/Actinium-project/acmd/rpcclient"
...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of Actinium Core wallet RPC server")
...
// Actinium transactions for these swaps.  A second tool should be used for the
...
// Scenerio 1:
//   cp1 initiates (dcr)
//   cp2 participates with cp1 H(S) (acm)
//   cp1 redeems acm revealing S
...
//     - must verify H(S) in contract is hash of known secret
//   cp2 redeems acm with S

func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: acmatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
type initiateCmd struct {
	cp2Addr *acmutil.AddressPubKeyHash
	amount  acmutil.Amount
}

type participateCmd struct {
	cp1Addr    *acmutil.AddressPubKeyHash
	amount     acmutil.Amount
	secretHash []byte
}
...
func run() (err error, showUsage bool) {
	...
	switch args[0] {
	case "initiate":
		cp2Addr, err := acmutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp2AddrP2PKH, ok := cp2Addr.(*acmutil.AddressPubKeyHash)
		if !ok {
		...
		amount, err := acmutil.NewAmount(amountF64)
		if err != nil {
			return err, true
		}
	...
	case "participate":
		cp1Addr, err := acmutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp1AddrP2PKH, ok := cp1Addr.(*acmutil.AddressPubKeyHash)
		if !ok {
...
// pubkey for a transaction input signature.  Due to limitations of the Actinium
// Core RPC API, this requires dumping a private key and signing in the client,
// rather than letting the wallet sign.
func createSig(tx *wire.MsgTx, idx int, pkScript []byte, addr acmutil.Address,
...
// acmd/rpcclient package.
func fundRawTransaction(c *rpc.Client, tx *wire.MsgTx, feePerKb acmutil.Amount) (fundedTx *wire.MsgTx, fee acmutil.Amount, err error) {
	...
	feeAmount, err := acmutil.NewAmount(resp.Fee)
	if err != nil {
		return nil, 0, err
	}
...
func getFeePerKb(c *rpc.Client) (useFee, relayFee acmutil.Amount, err error) {
	...
	relayFee, err = acmutil.NewAmount(netInfoResp.RelayFee)
	if err != nil {
		return 0, 0, err
	}
	payTxFee, err := acmutil.NewAmount(walletInfoResp.PayTxFee)
	if err != nil {
		return 0, 0, err
	}
	...
	if err == nil && estimateResp.FeeRate > 0 {
		useFee, err = acmutil.NewAmount(estimateResp.FeeRate)
		if relayFee > useFee {
			useFee = relayFee
		}
...
// account parameter which was removed in Actinium Core 0.15.
func getRawChangeAddress(c *rpc.Client) (acmutil.Address, error) {
	...
	addr, err := acmutil.DecodeAddress(addrStr, chainParams)
	if err != nil {
		return nil, err
	}
	...
	if _, ok := addr.(*acmutil.AddressPubKeyHash); !ok {
...
type contractArgs struct {
	them       *acmutil.AddressPubKeyHash
	amount     acmutil.Amount
	locktime   int64
	secretHash []byte
}
...
type builtContract struct {
	contract       []byte
	contractP2SH   acmutil.Address
	contractTxHash *chainhash.Hash
	contractTx     *wire.MsgTx
	contractFee    acmutil.Amount
	refundTx       *wire.MsgTx
	refundFee      acmutil.Amount
}
...
func buildContract(c *rpc.Client, args *contractArgs) (*builtContract, error) {
	...
	contractP2SH, err := acmutil.NewAddressScriptHash(contract, chainParams)
	if err != nil {
		return nil, err
	}
...
func buildRefund(c *rpc.Client, contract []byte, contractTx *wire.MsgTx, feePerKb, minFeePerKb acmutil.Amount) (
	refundTx *wire.MsgTx, refundFee acmutil.Amount, err error) {
	...
	refundAddr, err := acmutil.NewAddressPubKeyHash(pushes.RefundHash160[:], chainParams)
	if err != nil {
		return nil, 0, err
	}
	...
	if txrules.IsDustOutput(refundTx.TxOut[0], minFeePerKb) {
		return nil, 0, fmt.Errorf("refund output value of %v is dust", acmutil.Amount(refundTx.TxOut[0].Value))
	}
...
func calcFeePerKb(absoluteFee acmutil.Amount, serializeSize int) float64 {
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	recipientAddr, err := acmutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	contractHash := acmutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if sc == txscript.ScriptHashTy &&
			bytes.Equal(addrs[0].(*acmutil.AddressScriptHash).Hash160()[:], contractHash) {
			contractOut = i
			break
		}
	}
	...
	if txrules.IsDustOutput(redeemTx.TxOut[0], minFeePerKb) {
		return fmt.Errorf("redeem output value of %v is dust", acmutil.Amount(redeemTx.TxOut[0].Value))
	}
	...
	fmt.Printf("Redeem fee: %v (%0.8f ACM/kB)\n\n", fee, redeemFeePerKb)
...
func (cmd *auditContractCmd) runOfflineCommand() error {
	contractHash160 := acmutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if bytes.Equal(addrs[0].(*acmutil.AddressScriptHash).Hash160()[:], contractHash160) {
			contractOut = i
			break
		}
	}
	...
	contractAddr, err := acmutil.NewAddressScriptHash(cmd.contract, chainParams)
	if err != nil {
		return err
	}
	recipientAddr, err := acmutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
	...
	refundAddr, err := acmutil.NewAddressPubKeyHash(pushes.RefundHash160[:],
		chainParams)
	if err != nil {
		return err
	}
