...
var (
	...
	connectFlag = flagset.String("s", "localhost", "host[:port] of XSN Core wallet RPC server")
...
// xsncoin transactions for these swaps.  A second tool should be used for the
...
//   cp2 redeems xsn with S
//
// Scenerio 2:
//   cp1 initiates (btc)
//   cp2 participates with cp1 H(S) (xsn)
...
func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: xsnatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
// pubkey for a transaction input signature.  Due to limitations of the xsncoin
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Contract fee: %v (%0.8f XSN/kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.8f XSN/kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	fmt.Printf("Redeem fee: %v (%0.8f XSN/kB)\n\n", fee, redeemFeePerKb)
